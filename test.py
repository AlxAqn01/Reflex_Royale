...
#sound FX
pygame.mixer.music.load('assets/driftveil.ogg')
music_playing = False
...
def declare_winner():
    ...
    global player1_score, player2_score, prev_p1score, prev_p2score, invert_input
...
def handle_keypress(key):
    ...
    if not player_scored:
        ...
        # Check if Player 1 pressed a key
        if not player1_pressed and key in player1_keys.values():
            player1_pressed = True  # Player 1 has used their chance
            if not invert_input:
                if key == player1_keys[current_word]:  # word instead of color 
                    player_scored = True
                    player1_score += 1
            elif invert_input:
                if key == player1_keys[color_word]:  # color instead of word
                    player_scored = True
                    player1_score += 1

        # Check if Player 2 pressed a key
        if not player2_pressed and key in player2_keys.values():
            player2_pressed = True  # Player 2 has used their chance
            if key == player2_keys[current_word]:  # If the key was correct
                player_scored = True
                player2_score += 1
...
while Running:
    ...
    if app_state == MENU:
        if not music_playing:
            ...
            music_playing = True  # Mark music as playing
        menu_screen()
